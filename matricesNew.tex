...
%	\textcolor{red}{Above equation is not correct and must be rewritten as follows
%	\begin{equation*}
%		\overline{\mathbf{W}} \odot \mathbf{E}(\mathbf{e}_n, 0)\mathbf{W} = \frac{1}{N} \sum_a i^{-w(\mathbf{a})} E(\mathbf{a}, \mathbf{a}(\mathbf{S} + \mathbf{I})) \odot \sum_b i^{w(\mathbf{b}) - \mathbf{b}(\mathbf{S} + \mathbf{I})\mathbf{e}_n^T} E(\mathbf{b} + \mathbf{e}_n, \mathbf{b}(\mathbf{S} + \mathbf{I}))
%	\end{equation*}
%}
	...
	%\textcolor{red}{
	%	Also about $(b)$, we notice that result is $i^{-3 \mathbf{b}\mathbf{e}_n^T-\mathbf{b}\mathbf{S}^T-1}$ and considering mod 4 operation, we get the final expression.
	%}
	%\\
